...
function TemplateItem({ template, dragHandle }: { template: ExerciseTemplate; dragHandle: DragHandleProps }) {
  ...
  return <article className="template-card">
    ...
    <div className="template-actions"><span className="drag-hint">Pořadí změníte přetažením úchytky.</span><button className="text-danger compact" onClick={() => setConfirmDelete(true)} aria-label={`Odstranit výchozí cvik ${template.name}`}>Odstranit</button></div>
