...
type V2State = { version: 2; exerciseTemplates: ExerciseTemplate[]; workouts: Workout[]; activeWorkoutId: string | null }
export type LegacyAppState = Omit<V2State, 'version' | 'workouts'> & { version: 1; workouts: LegacyWorkout[] }

export const createInitialState = (): AppState => {
  const timestamp = new Date().toISOString()
  return { version: 3, workouts: [], activeWorkoutId: null,
...
export const isAppState = (value: unknown): value is AppState => isRecord(value) && value.version === 3 && hasWorkouts(value) && Array.isArray(value.workoutTemplates) && value.workoutTemplates.every(isWorkoutTemplate)
...
export const migrateV2State = (old: V2State): AppState => {
  const timestamp = new Date().toISOString()
...
export const loadState = (storage: Storage = localStorage): AppState => {
  ...
  try {
    ...
    if (isV2State(parsed) || isLegacyAppState(parsed)) {
      ...
      const migrated = migrateV2State(isLegacyAppState(parsed) ? migrateV1State(parsed) : parsed)
