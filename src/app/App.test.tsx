...
describe('uživatelské chování', () => {
  ...
  it('zobrazí název nad datem, přejmenuje trénink a promítne změnu do seznamu i úložiště', async () => {
    const user = userEvent.setup(); renderApp(); await chooseDefault(user)
    const heading = screen.getByRole('heading', { name: 'Výchozí trénink' })
    expect(heading.closest('.workout-name-section')?.nextElementSibling).toContainElement(screen.getByLabelText('Datum tréninku'))
    await user.click(screen.getByRole('button', { name: 'Přejmenovat' }))
    const input = screen.getByLabelText('Název tréninku')
    await user.clear(input); await user.click(screen.getByRole('button', { name: 'Uložit název' }))
    expect(screen.getByRole('alert')).toHaveTextContent('Zadejte název tréninku.')
    await user.type(input, '  Síla   A  '); await user.click(screen.getByRole('button', { name: 'Uložit název' }))
    expect(screen.getByRole('heading', { name: 'Síla A' })).toBeInTheDocument()
    await waitFor(() => { const stored = JSON.parse(localStorage.getItem(STORAGE_KEY)!) as AppState; expect(stored.workouts[0].name).toBe('Síla A'); expect(stored.workouts[0].sourceTemplateName).toBe('Výchozí trénink') })
    dispatchPopState(HOME_HISTORY_STATE)
    expect(await screen.findByRole('button', { name: /Síla A.*12 cviků/ })).toBeInTheDocument()
  })
...
describe('komponenty sérií podle osoby', () => {
  ...
  it('zobrazí stav cviku, po dokončení ho sbalí a otevře další vhodný cvik', async () => {
    ...
    await user.click(within(firstCard).getByRole('button', { name: 'Dokončit' }))
  ...
  it('umožní znovu otevřít dokončený cvik pro kontrolu hodnot', async () => {
    ...
    expect(toggle).toHaveAttribute('aria-expanded', 'true')
    expect(within(card).getByRole('region', { name: 'Lukáš' })).toBeInTheDocument()
    expect(within(card).getByRole('button', { name: 'Zrušit dokončení' })).toBeInTheDocument()
  })
  ...
  it('zrušení dokončení zachová data a ponechá otevřený pouze daný cvik', async () => {
    ...
    await user.click(within(firstCard).getByRole('button', { name: 'Dokončit' }))
    expect(within(secondCard).getByRole('button', { name: 'Dead bug' })).toHaveAttribute('aria-expanded', 'true')
    await user.click(within(firstCard).getByRole('button', { name: 'Kliky' }))
    expect(within(firstCard).getByRole('button', { name: 'Zrušit dokončení' })).toBeInTheDocument()
    await user.click(within(firstCard).getByRole('button', { name: 'Zrušit dokončení' }))
    ...
    expect(within(firstCard).getByRole('button', { name: 'Dokončit' })).toBeInTheDocument()
  })
