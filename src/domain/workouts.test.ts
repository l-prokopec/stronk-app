...
describe('vytvoření tréninku', () => {
  ...
  it('vloží pouze aktivní cviky', () => { const state = createInitialState(); state.exerciseTemplates[1].enabledByDefault = false; const workout = createWorkout(state); expect(workout.exercises).toHaveLength(11); expect(workout.exercises.some((item) => item.name === 'Dead bug')).toBe(false) })
  it('zachová pořadí šablon', () => { const state = createInitialState(); state.exerciseTemplates[0].order = 2; state.exerciseTemplates[2].order = 0; expect(createWorkout(state).exercises[0].name).toBe('Boční plank') })
