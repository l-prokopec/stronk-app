...
import type { PersonProfile, WorkoutExercise } from '../../types/workout'
import { PersonSetsEditor } from './PersonSetsEditor'

export function ExerciseCard({ workoutId, exercise, people, dragHandle, expanded, onToggle, onComplete }: { workoutId: string; exercise: WorkoutExercise; people: PersonProfile[]; dragHandle: DragHandleProps; expanded: boolean; onToggle: () => void; onComplete: () => void }) {
  const { dispatch } = useApp(); const [confirmRemove, setConfirmRemove] = useState(false)
  const hasValues = people.some((person) => exercise.setsByPerson[person.id].some((set) => set.weight || set.reps))
  ...
  return <article className="exercise-card">
    ...
    {expanded && <div className="exercise-card__content">
      {people.map((person) => <PersonSetsEditor key={person.id} workoutId={workoutId} exercise={exercise} person={person.id} displayName={person.name} color={person.color} />)}
