...
export function PersonSetsEditor({ workoutId, exercise, person, displayName, genitiveName, addName }: { workoutId: string; exercise: WorkoutExercise; person: Person; displayName: string; genitiveName: string; addName: string }) {
  ...
  return <section className={`person-sets-group person-sets-group--${person}`} aria-labelledby={headingId}>
    <h4 id={headingId}>{displayName}</h4>
    ...
    <button className="secondary full add-person-set" onClick={() => dispatch({ type: 'ADD_PERSON_SET', workoutId, exerciseId: exercise.id, person })}>+ Přidat sérii pro {addName}</button>
  </section>
}
