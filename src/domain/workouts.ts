...
export const createWorkout = (state: AppState, templateId: string | null = state.workoutTemplates[0]?.id ?? null, currentDate = new Date(), workoutId = createId()): Workout => {
  ...
  return {
    id: workoutId, name: source?.name ?? 'Prázdný trénink', date: toLocalDate(currentDate), sourceTemplateId: source?.id ?? null, sourceTemplateName: source?.name ?? 'Prázdný trénink', createdAt: timestamp, updatedAt: timestamp,
